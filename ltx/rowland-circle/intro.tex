...
\end{equation}
where
$\thetaB$ is the Bragg angle at the crystal centre (it is focused independently in the sagittal plane at a different distance, but this is not discussed here).
...
At the crystal \emph{centre}, this placement is exact: the incidence angle equals $\thetaB$
and the Bragg condition $\lambda = 2d\sin\thetaB$ is perfectly satisfied.
...
from $\thetaB$, and the Bragg condition is no longer perfectly satisfied.
...
$\delta E / E = -\cot\thetaB\,\delta\theta$, the corresponding relative energy broadening
is:
...
\begin{itemize}
  ...
  \item It vanishes at $\thetaB = 90°$ (backscattering), since $\cos 90° = 0$.
...
\begin{itemize}
  ...
  \item The crystal is tangent to the Rowland circle at $C$, and bent to radius
        $R = 2R_W$.
  \item The chord $SD$ subtends an angle $2\thetaB$ at the centre of the Rowland circle,
        consistent with Bragg's law.
\end{itemize}

As energy (and hence $\thetaB$) changes, the focus point $D$ moves continuously along the
...
\emph{planes} are still at angle $\thetaB$ to the incident beam.
